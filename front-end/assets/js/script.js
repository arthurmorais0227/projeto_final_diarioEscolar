  ...
  const dadosExemplo = {
    postagens: Array.from({ length: 10 }, (_, i) => {
      ...
      return {
        id: i + 1,
        autor: autor,
        descricao: `Postagem ${i + 1} do ${autor} - Uma reflexão interessante sobre desenvolvimento e tecnologia na turma 1TDS1`
      };
    })
  };
  ...
  function renderizarPostagens(postagens, origem = 'banco') {
    ...
    if (postagens && postagens.length > 0) {
      ...
      console.log(`✅ ${postagens.length} postagens carregadas da ${origem === 'banco' ? 'API' : 'memória (exemplo)'}`);
    } else {
      publicacoesContainer.innerHTML = '<p>Nenhuma postagem disponível no momento.</p>';
    }
  }
