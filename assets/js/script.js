...
function criarAviso(elemento, mensagem, duracao = 500) {
  ...
  // small delay to allow insertion before animating
  setTimeout(() => {
    aviso.style.opacity = "1";
    aviso.style.transform = "translateY(-25px)";
  }, 10);

  // hide after `duracao` ms
  setTimeout(() => {
...
// --- Docente indisponível: pop-out em elementos marcados como indisponivel ---
document.querySelectorAll(".img.indisponivel").forEach((el) => {
  el.style.cursor = "not-allowed";

  const mostrarAviso = () => {
    // evita criar múltiplos avisos simultâneos
    if (el.querySelector(".copiado-popup")) return;
    criarAviso(el, "Docente Indisponível", 1400);
  };

  el.addEventListener("mouseenter", mostrarAviso);
  el.addEventListener("click", mostrarAviso);
});
